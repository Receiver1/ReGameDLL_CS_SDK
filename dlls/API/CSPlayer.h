...
public:
    ...
    virtual void ClientCommand(const char *cmd,
    ...
    virtual void
        ReloadWeapons(CBasePlayerItem *pWeapon = nullptr, bool bForceReload = false, bool bForceRefill = false);
    ...
    virtual bool HintMessageEx(const char *pMessage,
                               float duration = 6.0f,
                               bool bDisplayIfPlayerDead = false,
...
public:
    ...
    DamageList_t &GetDamageList()
    {
        return m_DamageList;
    }
    void RecordDamage(CBasePlayer *pAttacker, float flDamage, float flFlashDurationTime = -1);
    int m_iNumKilledByUnanswered[MAX_CLIENTS]; // [0-31] how many unanswered kills this player has been dealt by each
                                               // other player
    bool m_bPlayerDominated[MAX_CLIENTS]; // [0-31] array of state per other player whether player is dominating other
                                          // players
